...
function JarvisModel() {
  ...
  return <primitive object={scene} scale={[0.7, 0.7, 0.7]} rotation={[0, 0, 0]} position={[0, 0, 0]} />;
}
...
export default function AIDiagnosis() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const [response] = await Promise.all([
        ...
        new Promise((resolve) => setTimeout(resolve, 10000)) // Ensures at least 10s delay
      ]);
  ...
  return (
    <div className="ai-container">
      <motion.div className="ai-3d-container" initial={{ x: "30vw" }} animate={{ x: 0 }} transition={{ duration: 1.5, ease: "easeInOut" }}>
      ...
      {showUI && (
        <motion.div className="ai-ui" initial={{ x: 50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: 1.5, ease: "easeInOut" }}>
  ...
  <div className="ai-result">
    ...
    {(displayedMode === 2 || displayedMode === 4 || displayedMode === 5) && (
      <p><strong>Note:</strong> No additional medical data available for this case.</p>
    )}

    <br />

    {/* ✅ Show "View & Download Report" button only for Case 1 & Case 3 */}
    {isAuthenticated && (displayedMode === 1 || displayedMode === 3) && (
      <button onClick={() => setIsModalOpen(true)} className="pdf-button">
